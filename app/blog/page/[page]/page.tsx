...
export function generateStaticParams() {
  const totalPages = getAllPostsPageCount(BLOG_POSTS_PER_PAGE)
  if (totalPages <= 1) {
    return []
  }
...
export async function generateMetadata({ params }: BlogPaginatedPageProps): Promise<Metadata> {
  ...
  const title = `블로그 ${page}페이지 | 노마드출장마사지`
  const description = `노마드출장마사지 블로그 ${page}페이지입니다. 경기 지역 출장마사지 정보와 이용 가이드를 확인할 수 있습니다.`
  const url = `https://www.nomadthai.kr/blog/page/${page}`
...
export default async function BlogPaginatedPage({ params }: BlogPaginatedPageProps) {
  ...
  const paginated = getPaginatedPosts(getSortedPostsData(), page, BLOG_POSTS_PER_PAGE)
